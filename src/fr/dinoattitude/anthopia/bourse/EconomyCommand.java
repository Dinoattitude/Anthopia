	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length == 0){
			...
			if(sender.hasPermission("anthopia.moderation.eco")) {
				if(args.length != 3) sender.sendMessage(Messages.INCOMPLETE_COMMAND.toString() + "/money add/remove/set <player> <amount>");
				else {
					if(!EconomyData.hasAccount(Bukkit.getPlayer(args[1]).getUniqueId())) { //TODO : Vérifier que l'ordre de la commande est respectée
						...
						if(target != null) {
							...
							else sender.sendMessage(Messages.INCORRECT_COMMAND.toString() + "/money add/remove/set <player> <amount>");
						}
						else sender.sendMessage(Messages.UNKNOWN_PLAYER.toString());
					}
				}
			}
		}
				...
				if(args.length != 2) sender.sendMessage(Messages.INCOMPLETE_COMMAND.toString() + "/money deposit/retire <amount>");
					...
					if(EconomyData.getBalance(pUuid) > amount && EconomyData.getBalance(pUuid) > 0) {
						if(args[0].equalsIgnoreCase("deposit")) {
							...
							else sender.sendMessage(Messages.NOT_ENOUGH_CB_MONEY.toString());
							
						}
						else sender.sendMessage(Messages.INCOMPLETE_COMMAND.toString() + "/money deposit/retire <amount>");
					}
					else
